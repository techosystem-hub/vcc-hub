...
export async function POST(req: Request) {
  ...
  try {
    ...
    const prompt = `You are a VC analyst reviewing publicly available information about a startup. Using the company identifiers and internal data below, write a concise 2-3 sentence overview of what this specific company does, the problem it solves, and why it may be relevant for an investor focused on ${investorCriteria.focusVerticals?.join(', ') || 'these verticals'} at ${investorCriteria.stagePreference?.join(', ') || 'early'} stage with a ${investorCriteria.ticketSize?.join(', ') || 'standard'} ticket size.

Draw on publicly available sources (website, LinkedIn, news) as well as all the structured data provided below.

Company data:
- Name: ${startup.startupName}
- Website / LinkedIn: ${startup.website}
- Jurisdiction: ${startup.jurisdiction || 'Not specified'}
- Email domain: ${startup.email ? '@' + startup.email.split('@')[1] : 'Not provided'}
- Sector / Verticals: ${startup.verticals?.join(', ') || 'Not specified'}
- Stage: ${startup.roundStage || 'Not specified'}
- Target raise: ${startup.targetRaise || 'Not specified'}
- Dual-use: ${startup.isDualUse || 'Not specified'}
- Description: ${startup.description || 'Not provided'}

Match context (from our algorithm):
- Match score: ${score}/100
- Match reasons: ${reasons?.join('; ') || 'None provided'}
